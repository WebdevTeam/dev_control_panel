...
SOURCE_FILE="${SELF_PATH/.newproject.sh/${CONFIG_FILE}}"
SOURCE_FILE_OVERRIDE="${SELF_PATH/.newproject.sh/${CONFIG_FILE_OVERRIDE}}"

INCLUDE_FLATURL="includes/flaturl.conf"
flaturl_path="${SELF_PATH/.newproject.sh/${INCLUDE_FLATURL}}"

SITEBASE_LOGIN="includes/sitebase_login.php"
orig_sitebase_login_file="${SELF_PATH/.newproject.sh/${SITEBASE_LOGIN}}"
...
usage="\n--------------New Project--------------\n"
usage+="USAGE: .newproject.sh [-h] [-f] [-p] [-l] [-v] [-y] [-u ssh_user] [-n svn_user] [-d deployment_file] [-s sitename]
...
if [ ! -f "$flaturl_path" ]; then
	...
	if [ "$repo_location" == "" ]; then
		...
		echo -e "\n-------------- REPO LOCATION ------------------"
		echo "Choose SVN Repo - Sitebase/Centraladmin/Privatesales/Logistics_api/Tmo/sKubase?"
...
if [ ! -z "$param_database_host" ] && [ "$param_database_host" != "-" ]; then
	...
	if [ $absolute_mode -eq 0 ]; then
		$svn_path info --username $svn_username $svnbranch_orignal &>/dev/null
	...
	if [ $branch_type != "trunk" ]; then
		if [ $orignal_branch_exists -eq 0 ]; then
			...
			read orignal_svnanswer
			if [ "$orignal_svnanswer" = "y" ] || [ "$orignal_svnanswer" = "Y" ];then
				branch_exists=0
				svnbranch="$svnbranch_orignal"
	...
	if [ $branch_exists -ne 0 ]; then
		commit_message=""
		branch_from=$svn_trunk_url

		if [ ! -z "$param_commit_message" ] && [ "$param_commit_message" != "-" ]; then
			commit_message=" $param_commit_message"
		fi

		if [ ! -z "$param_branch_from" ] && [ "$param_branch_from" != "-" ]; then

			if [[ $param_branch_from =~ ^http ]]; then
				branch_from="$param_branch_from"
			else
				branch_from="$svn_root_url/$param_branch_from"
			fi
		fi

		commit_message="$tracticket_message$commit_message"

		if [ "$commit_message" == "" ]; then
			commit_message=" $branch_name"
		fi

		tracticketmsg="[BRANCHING]: $commit_message"

		echo "[NEW BRANCH]: - $svnbranch"
		echo "[REPO]: - $branch_from"
		echo "[COMMIT MSG]: - '$tracticketmsg'"
		read -p "Okay to create? ( y/n ) : " svnanswer

		if [ "$svnanswer" = "y" ] || [ "$svnanswer" = "Y" ]
			then
			$svn_path copy $branch_from $svnbranch -m "$tracticketmsg"
			$svn_path info --username $svn_username $svnbranch &>/dev/null
			#Result is 0 if it exists, 1 if not
			newbranch_exists=$?
			if [ $newbranch_exists -ne 0 ]; then
				echo "There was an error creating the branch. Do you want to continue? ( y/n ) : \c"
				read continueanswer
				if [ "$continueanswer" = "n" ] || [ "$continueanswer" = "N" ]; then
					echo "BRANCH $svnbranch NOT CREATED"
					echo "\nEXITING"
					exit
				fi
			else
			echo "NEW BRANCH CREATED - $svnbranch FROM $branch_from"
			fi
		else
			echo "BRANCH $svnbranch NOT CREATED"
		fi
	else
		echo "NOTHING TODO - BRANCH $branch_name ALREADY EXISTS"
	fi


echo -e "\nUSING BRANCH NAME $branch_name"
...
#Result is 0 if it exists, 1 if not
newbranch_exists=$?
if [ $newbranch_exists -eq 0 ]; then
	if [ -d "$sites_directory" ]; then
		echo "NOTHING TODO - $sites_directory EXISTS"
	else
		echo "Do you want to check out a working copy of $svnbranch to $sites_directory? ( y/n ) : \c"
		read checkoutanswer
		if [ "$checkoutanswer" = "y" ] || [ "$checkoutanswer" = "Y" ]; then
			$svn_path co $svnbranch $sites_directory
			chown -R $owner_group $sites_directory
			echo "Working copy $svnbranch CHECKED OUT TO $sites_directory"
		else
			echo "Working copy $svnbranch NOT CHECKED OUT"

		fi
	fi
else
	echo -e "SKIPPING STEP 2 - branch $svnbranch DOES NOT EXIST\n"
fi


if [ -d "$sites_directory" ]; then
	echo -e "\nDo you want to setup a new project for $svnbranch? ( y/n ) : \c"
